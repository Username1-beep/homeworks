...
struct call_t {
  ...
  friend std::ostream &operator<<(std::ostream &stream, const call_t &a) {
    return stream << "call_t { number: " << a.number << ";  "
...
struct user_t {
  ...
  friend std::ostream &operator<<(std::ostream &stream, const user_t &a) {
    return stream << "user_t { number: " << a.number << ";  "
                  << "name: " << a.name << "; }";
  }
};
...
struct user_info_t {
  ...
  friend std::ostream &operator<<(std::ostream &stream, const user_info_t &a) {
    return stream << "user_info_t { user: " << a.user << ";  "
                  << "duration_s: " << a.total_call_duration_s << "; }";
  }
};
...
public:
   ...
   * If user with specified number was already existed, method should return
   ...
   * @return true if user with specified number exists and call-record was
   * actually added
   */
   ...
   * Return at most count call-record starts from start_pos (zero-indexed) in
   * ORDER
   * @param start_pos -- zero-indexed start position of call-records sorted in
   * ORDER
...
private:
  ...
  std::set<user_info_t, name_less> name_users{};
  ...
  std::map<std::string, std::set<user_info_t, number_less>> number_users;

  std::vector<call_t> calls{};

  user_info_t INVALID_USER{{"", ""}, -1};

  user_info_t user_by_number(const std::string &number);
};